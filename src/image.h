...
struct Image {
  ...
  Image(int w, int h, int c = 1) : w(w), h(h), c(c), data(nullptr) {
    ...
    if (w * h * c) data = (float*)calloc(w * h * c, sizeof(float));
  }
  ...
  const float* RowPtr(int row, int channel) const {
    return data + channel * w * h + row * w;
  }
  float* RowPtr(int row, int channel) {
    return data + channel * w * h + row * w;
  }
...
inline void save_png(const Image& im, const string& filename) {
  im.save_png(filename);
}
inline void save_image(const Image& im, const string& filename) {
  im.save_image(filename);
}
inline void save_binary(const Image& im, const string& filename) {
  im.save_binary(filename);
}
